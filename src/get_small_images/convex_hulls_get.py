...
def contours_from_binary(binary_image):
    ...
    # demo
    result_image_demo = 255 - np.zeros((binary_image.shape[0], binary_image.shape[1], 3), np.uint8)
    for i in range(len(contours)):
        if hierarchy[0, i, 3] == 0 or True:
            ...
            cv2.drawContours(result_image_demo, hull, i, color, 1, 8)
    cv2.imshow('img-windows', result_image_demo)
    cv2.waitKey(0)
...
def get_letter_images(gray_image, contours):
    ...
    for contour in contours:
        ...
        max_x = min([max_x + int((max_x - min_x) / 10), gray_image.shape[1] - 1])
        min_y = max([min_y - int((max_y - min_y) / 10), 0])
        max_y = min([max_y + int((max_y - min_y) / 10), gray_image.shape[0] - 1])

        cv2.rectangle(gray_image_copy, (min_x, min_y), (max_x, max_y), (0, 0, 255))
        letter_images.append(cv2.resize(
            gray_image[min_y:max_y, min_x:max_x],
            (28, 28)
        ))
    cv2.imshow('contours', gray_image_copy)
    cv2.waitKey(0)
